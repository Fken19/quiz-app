...
![アプリイメージ](docs/gemini_appimage.png)
...
- **フォーカス学習** — 「苦手だけ10問」「未学習だけ10問」など、弱点に集中した学習が可能（問題数を最大100問まで選択可能）
- **学習ステータス可視化** — 未学習/苦手/学習済み/得意を直近履歴から自動判定
- **語彙詳細ページ** — 各単語の学習履歴・例文・訳を確認でき、誤りを報告可能（カテゴリ別、レート制限あり）
...
- **招待コードプレビュー** — 生徒は招待コード入力時に講師情報を事前確認でき、承認制で紐付け申請
- **紐付け申請管理** — 講師ダッシュボードで pending 状態の申請を確認・承認/拒否でき、承認後の初期設定（表示名・タグ・メモ等）も一括編集可能
- **テスト作成・配信** — 任意の単語でテスト作成、作成済みテストは再利用可能
- **学習進捗の把握** — 生徒単位・クラス単位で学習状況を確認
- **プロフィール公開** — 生徒に対して講師の所属・自己紹介・アイコンを公開（紐付け済み生徒のみ閲覧可能）
...
- **ストレージ**: Google Cloud Storage（本番アバター画像保存、環境変数で切替可能）
- **CI/CD**: GitHub Actions
...
- `POST /api/invitation-codes/redeem/` - 招待コード適用（紐付け申請）
- `GET /api/student/teachers/{linkId}/` - 紐付け済み講師のプロフィール表示
...
- `GET /api/teacher/students/` - 紐付き生徒一覧（プロフィール情報含む）
...
- `GET /media/{path}` - メディアファイル配信（アバター画像等）
...
1. 生徒用URLにアクセス → （任意）「このアプリについて」で安心・プライバシー情報を確認 → Googleログイン
2. ユーザーホーム（塾管理下の場合は連絡・テスト・Streakを表示）
3. 招待コード入力 → 講師情報プレビュー確認 → 紐付け申請 → 講師承認待ち
4. 「英単語クイズに進む」→ レベル選択 → セクション選択（10問単位 or レベル内ランダム）
5. フォーカス学習カードから問題数を選択（未学習のみ、苦手のみ、最大100問）
6. クイズ開始（英単語 → 日本語4択、10秒タイマー）
7. 判定表示（◯/×/Timeout）→ どこでもタップで次へ
8. セクション結果表示 → 次のセクション or レベル一覧
9. ダッシュボードで学習量を可視化（日/週/月グラフ、Streak、ヒートマップ）
10. 語彙一覧・詳細で学習履歴を確認、誤りを報告
11. プロフィールで管理講師を確認・解除可能、講師詳細ページで公開情報を閲覧

### 講師側

1. 講師用URLにアクセス → Googleログイン + ホワイトリスト照合
2. 講師ホーム（生徒一覧、生徒追加、テスト作成、紐付け申請通知）
