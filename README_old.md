...
## 🚀 Django移行開始（2025-09-10〜）

このリポジトリは、Flask+FirestoreからDjango REST+Supabase(PostgreSQL)への移行を開始しました。

- backend/ : Django REST Framework + Supabase(PostgreSQL)
- frontend/ : Next.js（今後追加予定）
- legacy_flask/ : 旧Flaskアプリ（退避用、必要に応じて作成）

今後の開発は `migrate/django` ブランチで進めます。
