...
export default function TeacherTestNewPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      setSaving(true);
      setError(null);
      const response = await createTestWithQuestions({
        title: trimmedTitle,
        description: description.trim() || null,
        vocabulary_ids: selectedList.map((item) => item.vocabulary_id),
      });
      router.push(`/teacher/tests/${response.test_id}`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">テスト新規作成</h1>
          <p className="text-slate-600">語彙を選択して問題セットを作成します。</p>
        </div>
        <button
          type="button"
