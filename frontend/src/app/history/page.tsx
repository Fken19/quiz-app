...
export default function HistoryPage() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const url = qs ? `/quiz/history/?${qs}` : '/quiz/history/';
      const historyData = await apiGet(url);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/dashboard" className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md">
            ホームに戻る
          </Link>
        </div>
      </div>
  ...
  // 学習ダッシュボードへ移行済みのため、ここではサマリーは表示しない
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        {results.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">フィルター条件に一致する受験履歴がありません。</p>
            <Link href="/quiz/start" className="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
              クイズに挑戦する
            </Link>
          </div>
        ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <h4 className="text-lg font-medium text-gray-900">レベル{result.quiz_set.level} セグメント{result.quiz_set.segment}</h4>
                          <p className="text-sm text-gray-600">
                            {formatDate(result.quiz_set.started_at!)} • {result.total_questions}問 • {formatDuration(result.total_duration_ms)} • {result.quiz_set.mode === 'default' ? '順番通り' : 'ランダム'}
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-4">
                      <div className="text-right">
                        <p className="text-sm text-gray-600">{result.total_score}/{result.total_questions} 正解</p>
                        <p className="text-sm text-gray-600">平均 {(result.average_latency_ms / 1000).toFixed(1)}秒</p>
                      </div>
                      <Link href={`/quiz/${result.quiz_set.id}/result`} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 text-sm">
                        詳細
                      </Link>
                    </div>
                  </div>
